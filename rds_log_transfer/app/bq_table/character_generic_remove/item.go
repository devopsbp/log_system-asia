...
type CharacterGenericRemoveItem struct {
...
func (i CharacterGenericRemoveItem) Save() (map[string]bigquery.Value, string, error) {
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*CharacterGenericRemoveItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item CharacterGenericRemoveItem
		item.Record = record

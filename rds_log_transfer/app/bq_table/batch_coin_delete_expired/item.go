...
type BatchCoinDeleteExpiredItem struct {
...
func (i BatchCoinDeleteExpiredItem) Save() (map[string]bigquery.Value, string, error) {
	return map[string]bigquery.Value{
		"message_level":           i.MessageLevel,
		"message":                 i.Message,
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*BatchCoinDeleteExpiredItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item BatchCoinDeleteExpiredItem
		item.Record = record

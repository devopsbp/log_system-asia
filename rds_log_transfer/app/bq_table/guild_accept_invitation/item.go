package guild_accept_invitation
...
type GuildAcceptInvitationItem struct {
...
func (i GuildAcceptInvitationItem) Save() (map[string]bigquery.Value, string, error) {
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*GuildAcceptInvitationItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item GuildAcceptInvitationItem
		item.Record = record

package season_pass_change_points
...
type SeasonPassChangePointsItem struct {
	...
	ResultPoints          *json.Number          `json:"result_points,omitempty"`
...
func (i SeasonPassChangePointsItem) Save() (map[string]bigquery.Value, string, error) {
	return map[string]bigquery.Value{
		...
		"result_points":           i.ResultPoints,
		"change_date":             i.ChangeDate,
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*SeasonPassChangePointsItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item SeasonPassChangePointsItem
		item.Record = record

package second_password_registration
...
type SecondPasswordRegistrationItem struct {
...
func (i SecondPasswordRegistrationItem) Save() (map[string]bigquery.Value, string, error) {
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*SecondPasswordRegistrationItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item SecondPasswordRegistrationItem
		item.Record = record

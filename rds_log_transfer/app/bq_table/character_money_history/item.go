...
type CharacterMoneyHistoryItem struct {
	UserId                *string               `json:"user_id,omitempty"`
	CharacterId           *string               `json:"character_id,omitempty"`
	UpdateType            *string               `json:"update_type,omitempty"`
	ChangeAmount          *json.Number          `json:"change_amount,omitempty"`
	AfterAmount           *json.Number          `json:"after_amount,omitempty"`
	Action                *json.Number          `json:"action,omitempty"`
...
func (i CharacterMoneyHistoryItem) Save() (map[string]bigquery.Value, string, error) {
	return map[string]bigquery.Value{
		"user_id":                 i.Record.UserId,
		"character_id":            i.Record.CharacterId,
		"update_type":             i.UpdateType,
		"change_amount":           i.ChangeAmount,
		"after_amount":            i.AfterAmount,
		"action":                  i.Action,
...
func InsertBqItems(clientWithContext *bq.ClientWithContext, records *[]*shard_db.SbLogRecord, uuidSeed int) ([]*shard_db.SbLogRecord, error) {
	...
	var items = make([]*CharacterMoneyHistoryItem, 0)
	var failedRecords = make([]*shard_db.SbLogRecord, 0)
	for _, record := range *records {
		if tableName == "" {
			tableName = record.LogTag
		}
		var item CharacterMoneyHistoryItem
		item.Record = record
